...
def create_account_obj(json_part, api_part):
    ...
    new_account = Account(
        ...
        helper_details=[
            Helpers(
                builders_Apprentice_lvl=next(
                    (h.get("level", 0) for h in json_part.get("helpers", []) if h.get("data") == 93000000),
                    0,
                ),
                lab_Assistant_lvl=next(
                    (h.get("level", 0) for h in json_part.get("helpers", []) if h.get("data") == 93000001),
                    0,
                ),
                prospector_lvl=next(
                    (h.get("level", 0) for h in json_part.get("helpers", []) if h.get("data") == 93000003),
                    0,
                ),
                alchemist_lvl=next(
                    (h.get("level", 0) for h in json_part.get("helpers", []) if h.get("data") == 93000002),
                    0,
                ),
            )
        ],
